        ...
        if self.input_type.startswith('video'):
            ...
            if(args.start_frame == -1):
                if(args.end_frame == -1):
                    ...
                    # aud = meta['audio']
                    frame = meta['nb_frames']
                else:
                    self.stream_reader = (
                        ffmpeg.input(video_path).trim(end_frame = args.end_frame).setpts('PTS-STARTPTS').output('pipe:', format='rawvideo', pix_fmt='bgr24',
                                                        loglevel='error').run_async(
                                                            pipe_stdin=True, pipe_stdout=True, cmd=args.ffmpeg_bin))
                    # aud = (
                    #     ffmpeg.input(video_path).audio
                    #     .filter_('atrim', end_frame = args.end_frame)
                ...
                if(args.end_frame == -1):
                    self.stream_reader = (
                        ffmpeg.input(video_path).trim(start_frame = args.start_frame).setpts('PTS-STARTPTS').output('pipe:', format='rawvideo', pix_fmt='bgr24',
                                                        loglevel='error').run_async(
                                                            pipe_stdin=True, pipe_stdout=True, cmd=args.ffmpeg_bin))
                    # aud = (
                    #     ffmpeg.input(video_path).audio
                    #     .filter_('atrim', start_frame = args.start_frame)
                    #     .filter_('asetpts', 'PTS-STARTPTS')
                    # )
                    frame = meta['nb_frames'] - args.start_frame + 1
                else:
                    self.stream_reader = (
                        ffmpeg.input(video_path).trim(start_frame = args.start_frame, end_frame = args.end_frame).setpts('PTS-STARTPTS').output('pipe:', format='rawvideo', pix_fmt='bgr24',
                                                        loglevel='error').run_async(
                                                            pipe_stdin=True, pipe_stdout=True, cmd=args.ffmpeg_bin))
                    # aud = (
                    #     ffmpeg.input(video_path).audio
                    #     .filter_('atrim', start_frame = args.start_frame, end_frame = args.end_frame)
                    #     .filter_('asetpts', 'PTS-STARTPTS')
                    # )
                    frame = args.end_frame - args.start_frame
